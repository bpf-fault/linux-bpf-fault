...
def run_cmd(cmd, sudo=True):
    ...
    Simpler than run_command_with_live_output — no live streaming,
    just captures and returns.  Useful for scripts that parse output.
    """
...
# -------------------------------------------------------------------
#  bench_fault output parsing
#
#  Parses the human-readable output from print_results() in
#  bench_fault_util.h into structured dicts.
# -------------------------------------------------------------------

def parse_bench_output(stdout):
    """Parse print_results() output into a list of dicts (one per block).

    Each ``=== header ===`` section in the output becomes one dict with
    timing, context-switch, page-fault, and latency fields.
    """
    results = []
...
def _parse_block(text):
    ...
    if m:
        r["num_pages"] = int(m.group(1))

    r["verified"] = "Verification: OK" in text

    # Timings
    for label, key in [("Setup", "setup_ms"), ("Faults", "faults_ms"),
                        ("Teardown", "teardown_ms"), ("Total", "total_ms")]:
        m = re.search(rf'{label}:\s+([\d.]+)\s+ms', text)
        if m:
            r[key] = float(m.group(1))

    m = re.search(r'Throughput:\s+([\d.]+)\s+faults/sec', text)
    if m:
        r["throughput"] = float(m.group(1))

    # Context switches
    m = re.search(r'Voluntary:\s+(\d+)', text)
    if m:
        r["vol_csw"] = int(m.group(1))
    m = re.search(r'Involuntary:\s+(\d+)', text)
    if m:
        r["invol_csw"] = int(m.group(1))

    # Page faults
    m = re.search(r'Minor:\s+(\d+)', text)
    if m:
        r["minflt"] = int(m.group(1))
    m = re.search(r'Major:\s+(\d+)', text)
    if m:
        r["majflt"] = int(m.group(1))
    m = re.search(r'SIGSEGV:\s+(\d+)', text)
    if m:
        r["sig_faults"] = int(m.group(1))

    # Per-fault latency
    lat = {}
    for name in ["avg", "min", "max", "p50", "p99", "p999"]:
        m = re.search(rf'^\s+{name}:\s+(\d+)', text, re.MULTILINE)
        if m:
            lat[name] = int(m.group(1))
    if lat:
        r["latency_ns"] = lat
